// funcion encapsulada evita conflictos variables globales en .js
(function () {
    ...
    document.addEventListener("DOMContentLoaded", (e) => {
        refreshUI();
        lists.forEach((list) => {
            selectList.innerHTML += `<option value="${list.id}">${list.text}</option>`; //´´template literals´´
        });
    });
    ...
    form.addEventListener("submit", (e) => {
        e.preventDefault();
        const text = input.value.trim(); //trim() quita los ultimos espacios a los strings de  las respuestas?
    ...
    function renderTodos() {
        ...
        // para agregar eventos a los botos
    ...
    //en input type="checkbox literal + condicional de triada -- REACT   :else
    ...
    /* ------------------------------------ ^ ----------------------------------- */
    // genera ids aleatorios de manera mágica y misteriosa
    function uuidv4() {
